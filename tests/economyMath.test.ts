...
describe('explainAffordability', () => {
  it('returns null when credits and materials are sufficient', () => {
    ...
    const result = explainAffordability(state, getPlayerCorporation(state).id, home, 5000, [
  ...
  it('names insufficient credits clearly', () => {
    const state = newGame()
    getPlayerCorporation(state).credits = 100
    const result = explainAffordability(state, getPlayerCorporation(state).id, homeSystemId(state), 5000, [])
  ...
  it('names insufficient materials clearly', () => {
    const state = newGame()
    const home = homeSystemId(state)
    const result = explainAffordability(state, getPlayerCorporation(state).id, home, 0, [
